...
void FindPrimePolys(std::ostream* out, int fieldPower, int limit)
{
	...
	int primesFound = 0;
	LDPC_RS::RS_WORD fieldCharacteristic = ((RS_WORD)1 << fieldPower) - 1, fieldCharacteristicNext = ((RS_WORD)1 << (fieldPower + 1)) - 1;
...
void Poly::print()
{
	std::cout << "Poly(n=" << this->n << ")";
	if (this->n > 0)
	{
		std::cout << "[" << std::setw(3) << std::hex << (RS_WORD)this->coef[0];
	}
	for (int i = 1; i < this->n; i++)
	{
		std::cout << ", " << std::setw(3) << std::hex << (RS_WORD)this->coef[i];
	}
	if (this->n > 0)
	{
...
std::vector<uint8_t> Poly::get_vector() {
	if (data.size() > 0)
		return data;
	else {
		data.reserve(n);
		for (int i = 0; i < n; ++i)
			data.push_back(static_cast<uint8_t>(coef[i]));
		return data;
	}
}
