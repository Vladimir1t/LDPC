...
void LdpcCode::generate_gallagher_ldpc() {
    ...
    rand_perm.reserve(_N);
    // set some values:
    for (unsigned i = 0; i < _N; ++i) rand_perm.push_back(i); 
    ...
    for(unsigned i_row = 0; i_row < _M; ++i_row) {
        for(unsigned i_col = 0; i_col < _row_mat.at(i_row).size(); ++i_col) {
            if (_row_mat.at(i_row).at(i_col) < _K) {
                ...
                // std::cout << "i_row = " << i_row << "i_col = " << i_col <<'\n';
            }
            //  std::cout << "i_row = " << i_row << '\n';
        }
...
void LdpcCode::load_wifi_ldpc(unsigned block_length, unsigned rate_index) {
    ...
    switch(block_length) {
        ...
        case 1944: _Z = 81;
            break;
        default: 
            if (block_length % 24 == 0)
                _Z = block_length / 24;
            else {
                std::cout << "Block length value not supported for WiFi LDPC" << std::endl;
                _Z = block_length / 4;
            }
            break;
        //std::cout << "Block length value not supported for WiFi LDPC" << std::endl;
...
std::vector<std::vector<int>> LdpcCode::generate_ldpc_matrix(unsigned n, unsigned k) {
    unsigned m = n - k;
    unsigned Z = std::gcd(n, m);
    unsigned base_n = n / Z;
    unsigned base_m = m / Z;
    
    std::vector<std::vector<int>> baseH(base_n, std::vector<int>(base_m, -1));
    
    // Create a simple regular matrix (3 ones per column)
    for (unsigned col = 0; col < base_n; ++col) {
        int ones_added = 0;
        while (ones_added < 3) {
            unsigned row = rand() % base_m;
            if (baseH[col][row] == -1) {
                baseH[col][row] = rand() % Z; 
                ones_added++;
            }
        }
    }
